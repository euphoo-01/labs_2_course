...
IF ISNULL(@totalValue, 0) > 200
    BEGIN
        ...
        SELECT
            @totalValue   AS [Общая сумма значений],
...
SELECT TOP (5) * FROM dbo.[Значения показателей];
...
VALUES
    (2.0000,  4.0000, CAST((POWER(2.0000, 2) + POWER(4.0000, 2)) / NULLIF(2.0000 + 4.0000, 0) AS numeric(18,6))),
...
SELECT * FROM #L9_MYBASE_TEMP ORDER BY RowNumber;
