...
AS
SELECT P.[ID предприятия]       AS [Код],
...
AS
SELECT P.[Название предприятия] AS [Предприятие],
...
GO
SELECT *
FROM dbo.[Количество_показателей];
GO

CREATE VIEW dbo.[Важные_типы_показателей]
AS
SELECT T.[ID типа]             AS [Код],
...
GO
SELECT *
FROM dbo.[Важные_типы_показателей];
GO
...
GO
SELECT *
FROM dbo.[Показатели_предприятий]
ORDER BY [Предприятие], [Показатель], [Дата];
GO

CREATE VIEW dbo.[Типы_показателей]
AS
SELECT TOP (100) PERCENT T.[ID типа]             AS [Код],
                         T.[Название показателя] AS [Название показателя],
                         T.[Важность]            AS [Важность]
FROM dbo.[Типы показателей] AS T
...
SELECT OBJECT_SCHEMA_NAME(V.object_id) AS [Схема],
...
GO
SELECT *
FROM dbo.[Значения_показателей_по_датам]
