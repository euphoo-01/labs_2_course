...
namespace Garage
{
    class Car
    {
        ...
        private static bool ContainsModel(string model)
        {
            foreach (string availableModel in Models)
            {
        ...
        private static string[] GetYearsForModel(string targetModel)
        {
            foreach (var modelYears in ModelsYears)
            {
                if (modelYears.model == targetModel)
                    return modelYears.years;
            }

            return ModelsYears[ModelsYears.Length - 1].years; // нет даты
        }

        private static bool ContainsYear(string[] years, string targetYear)
        {
        ...
        public Car(string model, string color, string year)
        {
            ...
            string[] availableYears = GetYearsForModel(this.model);
            if (ContainsYear(availableYears, year))
                this.year = year;
            else
            {
        ...
        public string Year
        {
            ...
            set
            {
                string[] availableYears = GetYearsForModel(model);
                if (ContainsYear(availableYears, value))
                    year = value;
                else
                {
                    year = "Неизвестная дата производства";
        ...
        public static void PrintClassInfo()
        {
            ...
            Console.Write($"Доступные модели: ");
            for (int i = 0; i < Models.Length; i++)
            {
                Console.Write(Models[i]);
        ...
        {
            return $"Car [ID: {id}, Model: {model}, Year: {year}, Color: {color}, RegNumber: {regNumber}]";
        }
    ...
    class Program
    {
        static Car[] FilterByModel(Car[] cars, string targetModel)
        {
            List<Car> result = new List<Car>();
            foreach (Car car in cars)
            {
                if (car.Model == targetModel)
                    result.Add(car);
            }
        ...
        static Car[] FilterOldCars(Car[] cars, int yearsThreshold)
        {
            List<Car> result = new List<Car>();
            foreach (Car car in cars)
            {
                if (car.CalculateAge() > yearsThreshold)
                    result.Add(car);
            }
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"\ncar1 равна car2: {car1.Equals(car2)}");

            Car[] cars = [car1, car2, car3, testCar];

            string targetModel = "Nissan Skyline R34";
            Car[] filteredByModel = FilterByModel(cars, targetModel);
            ...
            Car[] oldCars = FilterOldCars(cars, yearsThreshold);
