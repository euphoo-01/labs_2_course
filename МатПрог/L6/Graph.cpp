...
namespace graph
{
    ...
    AMatrix::AMatrix(int n, int mr[])
    {
        this->n_vertex = n;
        this->mr = new int[n * n];
        for (int i = 0; i < n * n; i++) this->mr[i] = mr[i];
    }
    ...
    int  AMatrix::get(int i, int j)const { return this->mr[i*this->n_vertex+j]; }

    void AList::create(int n) { this->mr = new std::list<int>[this->n_vertex = n]; }
