...
using (IRepository repository = Repository.Create("Celebrities"))
{
    ...
    app.MapDelete("/Celebrities/{id:int}", (int id) =>
    {
        Celebrity? celebrity = repository.getCelebrityById(id);
        if (celebrity == null) throw new FoundByIdException($"Celebrity Id = {id}");
        if (!repository.delCelebrityById(id)) throw new DelCelebrityException("/Celebrities error, delCelebrityById() == false");
        if (repository.SaveChanges() <= 0) throw new SaveException("/Celebrities error, SaveChanges() <= 0");
        return celebrity;
    });
    ...
    {
        Celebrity? c = repository.getCelebrityById(id);
        if (c == null) throw new FoundByIdException($"Celebrity Id = {id}");
        if (!repository.updCelebrityById(id, celebrity)) throw new UpdCelebrityException("/Celebrities error, updCelebrityById() == false");
        if (repository.SaveChanges() <= 0) throw new SaveException("/Celebrities error, SaveChanges() <= 0");
    ...
    app.Map("/Celebrities/Error", (HttpContext ctx) =>
    {
        ...
        if (ex != null)
        {
            ...
            if (ex is DelCelebrityException) rc = Results.Problem(title: "ASPA004/delCelebrity", detail: ex.Message, instance: app.Environment.EnvironmentName, statusCode: 500);
...
public class DelCelebrityException : Exception { public DelCelebrityException(string message) : base($"delCelebrity error: {message}") { } }
public class UpdCelebrityException : Exception { public UpdCelebrityException(string message) : base($"updCelebrity error: {message}") { } }
